...
클라이언트 개발자가 쉽고 빠르고 효과적으로 AWS 환경을 구축 및 개발, 활용할 수 있도록 도와주는 SDK, CLI 및 콘솔환경을 제공 합니다.
...
서비스로 전달되는 모든 API요청을 처리하는 서비스로 클라이언트는 시스템 내부구조가 무엇인지 몰라도 서로 약속한 형태의 API를 호출하면 응답을 받을 수 있습니다.

API Gateway는 트래픽 관리, CORS 지원, 권한 부여 및 액세스 제어, 제한, 모니터링 및 API 버전 관리 등 최대 수십만 개의 동시 API 호출을 수신 및 처리하는 데 관계된 모든 작업을 처리합니다.

### 4. AWS Lambda

AWS Lambda는 서버 프로비저닝 또는 관리, 워크로드 인식 확장 로직 생성, 이벤트 통합 유지, 또는 런타임 관리 없이 코드를 실행할 수 있는 서버리스 컴퓨팅 서비스로 모든 유형의 애플리케이션이나 백엔드 서비스에 대한 코드를 별도의 관리 없이 실행할 수 있습니다. 사실상 AWS의 EC2와 더불어 가장 강조된느 서비스 중 하나 입니다.

### 5. AWS Dynamo DB

AWS의 완전 관리형 서비스로 어떤 규모에서도 10밀리초 미만의 성능을 제공하는 Key-Value 기반의 No-SQL 데이터베이스입니다.
